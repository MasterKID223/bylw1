...
def load_temporal_knowledge_graph(graph_name, idtype=settings.DGL_GRAPH_ID_TYPE):
    ...
    train_data_table = load_data_table(graph_name, train_file, column_names)
    val_data_table = load_data_table(graph_name, val_file, column_names)
    test_data_table = load_data_table(graph_name, test_file, column_names)
    all_data_table = pd.concat([train_data_table, val_data_table, test_data_table], ignore_index=True)

    stat_table = load_data_table(graph_name, "stat.txt", column_names=['num_entities', 'num_relations', '_'])
