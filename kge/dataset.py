...
class Dataset(Configurable):
    ...
    def __init__(self, config, folder=None):
        ...
        try:
            self._num_times: Int = config.get("dataset.num_times")
            if self._num_times < 0:
                self._num_times = None
        except KeyError:
            self._num_times: Int = None

        #: split-name to (n,4) int32 tensor
    ...
    def create_from(
        ...
        if "dataset" in checkpoint:
            ...
            dataset._num_times = dataset_checkpoint["num_tiems"]
        return dataset
    ...
    def _load_triples(filename: str, delimiter="\t", use_pickle=False) -> Tensor:
        ...
        triples = np.loadtxt(filename, usecols=range(0, 4), dtype=int)
    ...
    def shallow_copy(self):
        ...
        copy._num_times = self.num_times()
...
"""
    ...
    def num_times(self) -> int:
        "Return the number of times in this dataset."
        if not self._num_times:
            self._num_times = len(self.time_ids())
